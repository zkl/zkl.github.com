  ...
  StrSearch=StrSearch&"<form method=Get name=myform action=search.asp>"
   StrSearch=StrSearch&" <tr>"
     StrSearch=StrSearch&" <td height=28> <select name=Field size=1>"
      StrSearch=StrSearch&"    <option value=Title selected>产品名称</option>"
    ...
    StrSearch=StrSearch&"<tr>"
      StrSearch=StrSearch&"<td height=28>" 
       ...
       StrSearch=StrSearch&" <select name=BigClassName onChange=changelocation(document.myform.BigClassName.options[document.myform.BigClassName.selectedIndex].value) size=1>"
         StrSearch=StrSearch&" <option selected value=>所有大类</option>"

if not (rsBigClass.bof and rsBigClass.eof) then
	rsBigClass.movefirst
	do while not rsBigClass.eof
        StrSearch=StrSearch& "<option value='" & trim(rsBigClass("BigClassName")) & "'>" & trim(rsBigClass("BigClassName")) & "</option>"
	   	rsBigClass.movenext
	loop
end if
    ...
    StrSearch=StrSearch& "<tr>"
      StrSearch=StrSearch& "<td height=28>"
        ...
        StrSearch=StrSearch& "<select name=SmallClassName>"
          StrSearch=StrSearch& "<option selected value=''>所有小类</option>"
       StrSearch=StrSearch& " </select> "
        if ShowType=1 then
     StrSearch=StrSearch& " </td>"
   StrSearch=StrSearch& " </tr>"
   StrSearch=StrSearch& " <tr>"
     StrSearch=StrSearch& " <td height=28>" 
       ...
       StrSearch=StrSearch& " <input type=text name=keyword  size=12 value=关键字 maxlength=50 onFocus=this.select();>" 
