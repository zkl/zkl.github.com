...
strsmallclass=strsmallclass&"<TR><TD language=javascript onmouseup=""opencat(cat10"&i&"000,&#13;&#10; img10"&i&"000);"" id=item$pval[catID]) style=""CURSOR: hand"" width=34 height=24 align=center><IMG id=img10"&i&"000 src=img/class1.gif width=20 height=20></TD><TD width=662><a href='List_"&rsbig("BigClassID")&"_1.html'>"&rsbig("enBigClassName")&"</a></TD></TR><TR>"
...
strsmallclass=strsmallclass&"&nbsp;<IMG height=20 src=img/class3.gif width=26 align=absMiddle border=0><a href=Slist_"&rsSmall("SmallClassId")&"_1.html>" & rsSmall("enSmallClassName") & "</a><BR>"
...
<%StrSearch="<table border=0 cellpadding=2 cellspacing=0 align=center>"
  StrSearch=StrSearch&"<form method=Get name=myform action=ensearch.asp>"
   StrSearch=StrSearch&" <tr>"
     StrSearch=StrSearch&" <td height=28> <select name=Field size=1>"
      StrSearch=StrSearch&"    <option value=Title selected>Title</option>"
     StrSearch=StrSearch&"     <option value=Content>Content</option>"
...
if ShowType=1 then
    ...
    StrSearch=StrSearch&"<tr>"
      StrSearch=StrSearch&"<td height=28>" 
       ...
       StrSearch=StrSearch&" <select name=BigClassName onChange=changelocation(document.myform.BigClassName.options[document.myform.BigClassName.selectedIndex].value) size=1>"
         StrSearch=StrSearch&" <option selected value=>All Bigclass</option>"

if not (rsBigClass.bof and rsBigClass.eof) then
	rsBigClass.movefirst
	do while not rsBigClass.eof
        StrSearch=StrSearch& "<option value='" & trim(rsBigClass("enBigClassName")) & "'>" & trim(rsBigClass("enBigClassName")) & "</option>"
	   	rsBigClass.movenext
	loop
end if
    ...
    StrSearch=StrSearch& "<tr>"
      StrSearch=StrSearch& "<td height=28>"
        ...
        StrSearch=StrSearch& "<select name=SmallClassName>"
          StrSearch=StrSearch& "<option selected value=''>All Smallclass</option>"
       StrSearch=StrSearch& " </select> "
        if ShowType=1 then
     StrSearch=StrSearch& " </td>"
   StrSearch=StrSearch& " </tr>"
   StrSearch=StrSearch& " <tr>"
     StrSearch=StrSearch& " <td height=28>" 
       ...
       StrSearch=StrSearch& " <input type=text name=keyword  size=12 value=Keyword maxlength=50 onFocus=this.select();>" 
        StrSearch=StrSearch& "<input type=submit name=Submit  value=Search> </td>"
   StrSearch=StrSearch& " </tr>"
...
if rsBigClass.bof and rsBigClass.eof then 
		...
		strClassName=""
		do while not rsBigClass.eof
			strClassName=strClassName & "【<a href='Slist_" & rsBigClass("bigclassid") & "_1.html'><b>" & rsBigClass("enBigClassName") & "</b></a>】<br><br>"
			sqlClass="select * from SmallClass where BigClassName='" & rsBigClass("BigClassName") & "' Order by SmallClassID"
			Set rsClass= Server.CreateObject("ADODB.Recordset")
			rsClass.open sqlClass,conn,1,1
			do while not rsClass.eof
				strClassName=strClassName & "&nbsp;<a href='List_" & rsClass("SmallClassid") & "_1.html'>" & rsClass("enSmallClassName") & "</a>&nbsp;"
				rsClass.movenext
			loop
...
sub ShowUserLogin()
	...
	If Session("UserName")="" Then
		...
		strLogin=strLogin & "<form action='EnUserLogina.asp' method='post' name='UserLogin' onSubmit='return CheckForm();'>"
