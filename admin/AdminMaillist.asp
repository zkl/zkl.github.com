...
sub DoExportList()
	...
	case 1
		...
		tConn.close
		Set tconn = Nothing
		strResult="操作成功：共导出 "& i &" 个用户Email地址到数据库 "&tdb&"。<a href="&ExportFileName&">点击这里将数据库新闻回本地</a>"
	case 2
		...
		Application.unlock
		set fso=nothing
		strResult="操作成功：共导出 " & i & " 个用户Email地址到"&ExportFileName&"文件。<a href="&ttxt&">点击这里将文件新闻回本地</a>)"
	end select
	rs.close
	set rs=nothing
%>
