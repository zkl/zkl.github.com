...
FirstImageName=trim(request("FirstImageName"))
Action=trim(request("Action"))
if BigClassName="" then
	founderr=true
	errmsg=errmsg+"<li>未指定项目所属大类</li>"
end if
if Title="" then
	founderr=true
	errmsg="<li>项目标题不能为空</li>"
end if
if Content="" then
	founderr=true
	errmsg=errmsg+"<li>项目内容不能为空</li>"
end if
	 ...
	 case "Modify"	
  		if ID<>"" then
			...
			if not (rs.bof and rs.eof) then
				...
				founderr=true
				errmsg=errmsg+"<li>找不到此项目，可能已经被删除。</li>"
				call WriteErrMsg()
			end if
		else
			founderr=true
